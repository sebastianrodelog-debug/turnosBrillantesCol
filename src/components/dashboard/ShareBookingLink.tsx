...
export function ShareBookingLink() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Reserva tu turno en ${business?.name || 'nuestro negocio'}`,
          text: `Agenda tu cita fácilmente en ${business?.name || 'nuestro negocio'}`,
          url: bookingUrl,
        });
      } catch {
  ...
  const handleWhatsAppShare = () => {
    const message = encodeURIComponent(
      `¡Hola! Reserva tu turno en ${business?.name || 'nuestro negocio'}: ${bookingUrl}`
    );
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Comparte este enlace con tus clientes para que puedan agendar turnos.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="border border-dashed border-border rounded-lg p-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Próximamente: Código QR para imprimir
            </p>
          </div>
        </div>
      </DialogContent>
